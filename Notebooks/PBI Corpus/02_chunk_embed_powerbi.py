...
# MAGIC %md
# MAGIC # 02 - Chunk & Embed: Power BI Docs Corpus
# MAGIC ## Databricks Expert Assistant Project
# MAGIC
# MAGIC This notebook chunks and embeds the Power BI raw documentation corpus created by Notebook 1.
# MAGIC
# MAGIC **Input:** `chatbot.rag_chatbot.raw_docs_powerbi`  
# MAGIC **Output:** `chatbot.rag_chatbot.doc_chunks_powerbi`
# MAGIC
# MAGIC The current Databricks app chunk table is intentionally left untouched. Power BI chunks land separately first so we can validate quality, preserve product metadata, and avoid cross-product retrieval contamination.
...
# MAGIC
# MAGIC Use a Power BI-specific chunk table first. Later, once the app and Vector Search index are product-aware, we can merge Databricks and Power BI chunks into a unified table/index.
...
CHUNKS_TABLE = os.getenv("POWERBI_CHUNKS_TABLE", f"{CATALOG}.{SCHEMA}.doc_chunks_powerbi")
...
print(f"Power BI chunk target: {CHUNKS_TABLE}")
...
# MAGIC
# MAGIC Confirm the Power BI raw table exists and contains the metadata needed for product-aware retrieval.
...
# MAGIC
# MAGIC Older raw tables may not have all product-aware columns. This keeps the Power BI path resilient while making the output schema explicit.
...
if "doc_family" not in raw_df.columns:
    raw_df = raw_df.withColumn("doc_family", lit("powerbi"))
else:
    raw_df = raw_df.withColumn("doc_family", coalesce(col("doc_family"), lit("powerbi")))
...
# MAGIC
# MAGIC Fail fast before embedding the whole Power BI corpus.
...
try:
    test_response = deploy_client.predict(
        ...
        inputs={"input": ["What is Power BI OneLake and how does it work?"]},
    )
...
print(f"Embedding {chunk_count:,} Power BI chunks...")
...
if embedded_count == 0:
    raise RuntimeError("No Power BI chunks were embedded successfully.")
...
# MAGIC
# MAGIC This writes the Power BI-specific chunk table and leaves the existing Databricks `doc_chunks` table alone.
...
# MAGIC
# MAGIC Keep this disabled until the app retrieval code and Vector Search index can filter or rank by `product`. Once ready, this can combine Databricks and Power BI chunks into one product-aware table.
...
if BUILD_UNIFIED_CHUNKS_TABLE:
    ...
    powerbi_chunks_df = spark.table(CHUNKS_TABLE)
    unified_df = databricks_chunks_df.unionByName(powerbi_chunks_df, allowMissingColumns=True)
...
run_id = datetime.utcnow().strftime("powerbi_embed_%Y%m%d_%H%M%S")
...
# MAGIC ## Next Steps
# MAGIC
# MAGIC 1. Run this notebook after `raw_docs_powerbi` exists.
# MAGIC 2. Create or refresh a Vector Search index for `doc_chunks_powerbi`.
# MAGIC 3. Update the Streamlit app to route Fabric questions to Power BI chunks.
