...
# MAGIC %md
# MAGIC # 03 - Vector Search: Power BI Docs Corpus
# MAGIC ## Databricks Expert Assistant Project
# MAGIC
# MAGIC This notebook creates or refreshes a Mosaic AI Vector Search Delta Sync index for the Power BI chunk table.
# MAGIC
# MAGIC **Input:** `chatbot.rag_chatbot.doc_chunks_powerbi`  
# MAGIC **Output index:** `chatbot.rag_chatbot.doc_chunks_powerbi_index`
# MAGIC
# MAGIC The Vector Search endpoint can be shared with the Databricks index, but the Power BI index stays separate until app routing is product-aware.
...
# MAGIC
# MAGIC 1. `01_ingestion_powerbi`
# MAGIC 2. `02_chunk_embed_powerbi`
# MAGIC 3. `03_vector_search_powerbi`
# MAGIC
# MAGIC Keep the Power BI index separate first. Later, a unified product-aware index can be introduced after app routing validates `product = powerbi` vs `product = databricks` filtering.
...
VS_INDEX = os.getenv("POWERBI_VECTOR_SEARCH_INDEX", f"{CATALOG}.{SCHEMA}.doc_chunks_powerbi_index")
...
print(f"Power BI index    : {VS_INDEX}")
...
# MAGIC
# MAGIC Confirm the Power BI chunk table exists, has embeddings, and preserves product-aware metadata.
...
# MAGIC
# MAGIC The endpoint is compute for similarity search. It can safely host both Databricks and Power BI indexes.
...
# MAGIC
# MAGIC The Power BI index is separate from the Databricks index, but uses the same embedding column and primary key pattern.
...
if RUN_MANUAL_SYNC:
    ...
    for attempt in range(1, 4):
        try:
            print(f"Triggering Power BI Vector Search index sync, attempt {attempt}...")
            idx.sync()
...
# MAGIC
# MAGIC This tests the Power BI index after sync. This index stores precomputed embeddings, so the smoke test embeds the query first and searches with `query_vector`.

# COMMAND ----------

try:
    query_vector = embed_query_text("What is Power BI OneLake?")
...
run_id = datetime.utcnow().strftime("powerbi_vs_%Y%m%d_%H%M%S")
