...
LOCAL_STORAGE_SESSIONS_KEY = "dbx_expert_all_sessions"
LOCAL_STORAGE_BRIDGE_KEY = "_ls_history_bridge"
LOCAL_STORAGE_SESSIONS_BRIDGE_KEY = "_ls_sessions_bridge"
MAX_PERSISTED_MESSAGES = 50
MAX_CHAT_SESSIONS = 20
MAX_SIDEBAR_CHAT_SESSIONS = 10


def _local_storage_now_iso() -> str:
    return datetime.now(timezone.utc).isoformat()


def _slim_chat_messages(messages: list) -> list:
    slim = []
...
def _normalize_chat_session(session: dict):
    ...
    created_at = session.get("created_at") or session.get("updated_at") or _local_storage_now_iso()
    ...
    return {
        "id": session_id,
        "title": title,
        "messages": _slim_chat_messages(messages),
        "created_at": created_at,
        "updated_at": updated_at,
    }


def _sort_chat_sessions(sessions: list) -> list:
...
def _ensure_chat_sessions_state() -> list:
    ...
    for session in sessions:
        normalized_session = _normalize_chat_session(session)
    ...
    normalized = _sort_chat_sessions(normalized)[:MAX_CHAT_SESSIONS]
    st.session_state["chat_sessions"] = normalized
    return normalized


def _upsert_chat_session(messages: list, session_id: str) -> list:
    ...
    slim_messages = _slim_chat_messages(messages)
    ...
    if session_id and (slim_messages or existing_session):
        timestamp = _local_storage_now_iso()
        sessions.append(
            {
                "id": session_id,
    ...
    sessions = _sort_chat_sessions(sessions)[:MAX_CHAT_SESSIONS]
    st.session_state["chat_sessions"] = sessions
    return sessions


def _write_chat_sessions_to_local_storage(sessions: list, current_messages: list, session_id: str):
    components.html(
        f"""<script>
        try {{
            window.parent.localStorage.setItem("{LOCAL_STORAGE_SESSIONS_KEY}", {json.dumps(json.dumps(sessions))});
            window.parent.localStorage.setItem("{LOCAL_STORAGE_KEY}", {json.dumps(json.dumps(current_messages))});
...
def save_chat_to_local_storage(messages: list, session_id: str = ""):
    ...
    slim_messages = _slim_chat_messages(messages)
    sessions = _upsert_chat_session(messages, active_session_id)
    _write_chat_sessions_to_local_storage(sessions, slim_messages, active_session_id)
...
def inject_history_loader():
    ...
    components.html(
        ...
        (function() {{
            ...
            const activeSessionId = window.parent.localStorage.getItem("{LOCAL_STORAGE_SESSION_KEY}") || "";
            const sessionsStored = window.parent.localStorage.getItem("{LOCAL_STORAGE_SESSIONS_KEY}");
            let payload = null;
            ...
            if (!payload) {{
                const storedMessages = window.parent.localStorage.getItem("{LOCAL_STORAGE_KEY}");
            ...
            const bridge = window.parent.document.querySelector('textarea[aria-label="{LOCAL_STORAGE_SESSIONS_BRIDGE_KEY}"]');
            if (!bridge) return;
...
def render_history_bridge():
    ...
    st.text_area(
        ...
        key=LOCAL_STORAGE_SESSIONS_BRIDGE_KEY,
        value="",
        label_visibility="collapsed",
        height=1,
    )
...
def try_restore_from_local_storage() -> list:
    ...
    sessions_bridge_value = st.session_state.get(LOCAL_STORAGE_SESSIONS_BRIDGE_KEY, "")
    if sessions_bridge_value and sessions_bridge_value.strip():
        try:
            ...
            normalized_sessions = _sort_chat_sessions(normalized_sessions)[:MAX_CHAT_SESSIONS]
            if normalized_sessions:
                ...
                st.session_state[LOCAL_STORAGE_SESSIONS_BRIDGE_KEY] = ""
...
def get_chat_sessions_for_sidebar(limit: int = MAX_SIDEBAR_CHAT_SESSIONS) -> list:
        ...
        sessions.append(
            {
                ...
                "messages": _slim_chat_messages(current_messages),
                "created_at": timestamp,
                "updated_at": timestamp,
            }
        )

    sessions = _sort_chat_sessions(sessions)
...
def clear_local_storage():
    ...
    components.html(
        f"""<script>
        window.parent.localStorage.removeItem("{LOCAL_STORAGE_SESSIONS_KEY}");
